...
keypad_frame = tk.LabelFrame(root_frame, width=400,height=600)
keypad_frame.grid(row=1,column=0,padx=10,pady=(10,10))
keypad_frame.grid_propagate(0)

numbers_frame = tk.LabelFrame(keypad_frame,width=260,height=570)
numbers_frame.grid(row=0,column=0,padx=(10,20),pady=(10,10))
numbers_frame.grid_propagate(0)

signs_frame = tk.LabelFrame(keypad_frame,width=100,height=570)
...
#add number button 
button_1 = tk.Button(numbers_frame,text="1",height=3,width=3,command=lambda num="1":show("1"))
button_1.grid(row=0,column=0,padx=10,pady=10)

button_2 = tk.Button(numbers_frame,text="2",height=3,width=3,command=lambda num="2":show("2"))
...
button_9 = tk.Button(numbers_frame,text="9",height=3,width=3,command=lambda num="9":show("9"))
...
#adding symbols 
button_add = tk.Button(signs_frame, text="+",width=3,height=3,command=lambda sign="+": assign("+"))
button_add.grid(row=0,column=0,padx=(10,10),pady=10)

button_subtract = tk.Button(signs_frame, text="-",width=3,height=3,command=lambda sign="-": assign("-"))
button_subtract.grid(row=1,column=0,padx=(10,10),pady=10)

button_multiply = tk.Button(signs_frame, text="*",width=3,height=3,command=lambda sign="*": assign("*"))
button_multiply.grid(row=2,column=0,padx=(10,10),pady=10)

button_divide = tk.Button(signs_frame, text="/",width=3,height=3,command=lambda sign="/" : assign("/"))
button_divide.grid(row=3,column=0,padx=(10,10),pady=10)

#applying font to the button
button_font = font.Font(size = 20,weight="bold",family ="Helventica")
